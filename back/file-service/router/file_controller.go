...
func upload(router *gin.Engine) {
	router.POST("/upload", func(c *gin.Context) {
		...
		logger.Info("File uploaded {g}%s{;} size :%d ", f.Filename, f.Size)
		file, err := f.Open()
		if err != nil {
			logger.Warn("{_r}File open errpr{;} :%s", err.Error())
			c.JSON(200, gin.H{"errno": 1, "message": "上传失败,请稍后再试.."})
			return
		}
		path, err := aliyun.Upload(file, f.Filename)
		if err != nil {
			logger.Warn("{_y}oss upload failed{;} :%s", err.Error())
			c.JSON(200, gin.H{"errno": 1, "message": "上传失败,请稍后再试.."})
			return
		}
...
func deleteFile(router *gin.Engine) {
	router.DELETE("/delete", func(c *gin.Context) {
		...
		if err := c.ShouldBindJSON(&data); err != nil {
			logger.Warn("{_r}ShouldBindJSON in deleteFile error{;} :%s", err.Error())
		...
		if err != nil {
			logger.Warn("{_r}oss delete error{;} :%s", err.Error())
			c.JSON(200, gin.H{"errno": 1, "message": "删除失败,请稍后重试!"})
			return
		}
